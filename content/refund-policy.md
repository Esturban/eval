---
title: "Refund Policy"
date: 2026-09-25T00:00:00+00:00
draft: false
language: en
description: Refund policy for EV Advisory products.
---

Last updated 2026-09-23.

Every product carries its own written guarantee. The guarantee on the product you bought, shown on the [store page]({{< relref "store" >}}) at the time of purchase, is the one that applies.

## Competitor Review Objection Radar, USD 897

If the memo does not name at least 3 objections your team can act on this week, full refund within 5 business days of delivery.

## AI Security Posture Audit, USD 9,000

You see the full risk classification method and scoring rubric before the second payment is due. If the posture map does not surface at least one finding your team did not already know about, the second payment is waived.

## Call Recording Retention and Deletion Audit, USD 4,500

If the deletion path matrix does not show at least one gap between your written policy and what your vendor actually does, there is no invoice.

## RFP and RFI Response Automation, SAP Ariba, USD 24,000

You see the working compliance matrix and a first drafted section by the end of week 2. If the pipeline does not cut your team's first-draft response time by at least 30% on the pilot tender, the week 4 balance is waived.

## How a refund is paid

A refund is issued to the original payment method through Stripe. Where the guarantee waives a milestone payment, that payment is simply not invoiced; no separate refund step is needed.

## Requesting a refund

Reply to your receipt or booking confirmation email. State which guarantee condition applies. EV Advisory confirms and processes the refund within 5 business days of the request.
