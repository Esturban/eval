...
## Motion
- **Approach:** One continuous, scroll-driven Three.js scene pinned behind the entire homepage story - hero through Reviews - with a single light closing CTA at the very end; minimal-functional elsewhere.
- **Hero (2026-09-24 finish-gate rework):** the separate static hero plate (poster SVG + its own dark shell) is retired. The pinned agent-network canvas IS the hero now; the headline/subtitle/CTA (`hero-intro.html`) render as plain text over it, no second boxed shell or illustrated scene competing with it.
- **Agent network background:** a sticky (`position: sticky`) WebGL canvas pinned behind the hero, Mission, Tools & Integrations, Validation & Proof, and Reviews & Confirmations sections. It shows four honestly-labeled agents (Inbox agent, CRM agent, Project agent, Docs agent, matching the real tool list in `content/services/ai-agent-implementation.md`), each an icosahedron with a slow-rotating ring, positioned on the same straight-lane + on-ramp geometry as before. Instead of one courier agent visiting static tool icons, a task/tool/message glyph now hands directly from each agent to the next in sequence (three handoffs, one in flight at a time) - the literal agent-to-agent protocol EV asked for. A small set of dim ambient points streams the length of the shared lane continuously, independent of scroll, for a sustained "always moving" feel; the camera also gets a tiny continuous idle drift on top of its scroll-driven path. Camera and story state are still driven entirely by real scroll position (`agent-network-loader.js`), with one addition: the hero+mission scroll range is now measured at runtime and reported as a flat "prologue" (story progress pinned at 0) so the camera/label/handoff timing tuned for the Tools-Proof-Reviews span isn't compressed by the extra height above it.
- **Lazy-load and fallback:** unchanged - the heavy three.js-containing bundle (`agent-network-scene.js`) is only fetched via a dynamic `import()` once the section nears the viewport (`IntersectionObserver`, 600px root margin); the tiny loader (`agent-network-loader.js`) ships unconditionally. `prefers-reduced-motion: reduce` visitors, browsers without `IntersectionObserver`, and no-JS/failed-load cases all see a static inline SVG poster instead and never fetch the WebGL bundle at all.
- **Rendering budget:** unchanged - MeshStandardMaterial (not physically-based transmission), no post-processing/bloom pass, three lights, four low-poly nodes plus a handful of small additive sprites for handoffs and the ambient stream, pixel ratio capped at 1.75, and the render loop pauses via a second `IntersectionObserver` whenever the section scrolls out of view.
- **Nav:** the site nav is sticky at all breakpoints and keeps its three homepage-only anchor links (Tools / Proof / Reviews) in the same pill style as the existing Store link.
- **Restraint pass (2026-09-24):** the Mission, Tools, Validation-stats, and Reviews card grids dropped their filled `bg-slate-900/80` boxes for a plain top-rule (`border-t`) treatment, so the page reads as fewer competing boxed panels stacked against the scene behind it. Copy in those same sections was compressed from walkthrough-style paragraphs to short "what we do" lines (facts unchanged, no new claims). The two real case-study cards and the Live Audit product card stay boxed - they're distinct functional/clickable units, not decorative repetition.
